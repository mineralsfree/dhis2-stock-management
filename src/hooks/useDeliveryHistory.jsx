...
export const useDeliveryHistory = ()=>{
  ...
  const {loading, error, data, refetch: internalRefetch} = useDataQuery(deliveryHistoryQuery);
  ...
  if (data && !deliveryHistory && !refetchLoading){
      console.log('useDeliveryHistory', data);
      setDeliveryHistory(Object.values(data.deliveryHistory.deliveryHistory));
  }
  return {loading: loading || refetchLoading, error, deliveryHistory, refetch};
}